...
#define SPIS_SPI_TX_DATA_BITS_NUM       (8u)
...
#define SPIS_SPI_RX_BUFFER_SIZE         (8u)
#define SPIS_SPI_TX_BUFFER_SIZE         (8u)

#define SPIS_SPI_INTERRUPT_MODE         (1u)

#define SPIS_SPI_INTR_RX_MASK           (0u)
...
****************************************/
    ...
    #define SPIS_CHECK_SPI_WAKE_ENABLE  ((0u != SPIS_scbEnableWake) && SPIS_SCB_MODE_SPI_RUNTM_CFG)
    #define SPIS_CHECK_UART_WAKE_ENABLE ((0u != SPIS_scbEnableWake) && SPIS_SCB_MODE_UART_RUNTM_CFG)
    ...
    /* Internal RX and TX buffer: for SPI or UART */
        ...
        /* Get FIFO size */
        #define SPIS_SPI_UART_FIFO_SIZE SPIS_GET_FIFO_SIZE(SPIS_SPI_BYTE_MODE_ENABLE)
        ...
        /* Get FIFO size */
        #define SPIS_SPI_UART_FIFO_SIZE SPIS_GET_FIFO_SIZE(SPIS_UART_BYTE_MODE_ENABLE)
    ...
    #define SPIS_CHECK_UART_WAKE_ENABLE (SPIS_UART_WAKE_ENABLE_CONST)
...
{   
    ...
    */
     ...
     *  choices: 
     *  - SPIS_SPI_MODE_MOTOROLA 
     *  - SPIS_SPI_MODE_TI_COINCIDES
     *  - SPIS_SPI_MODE_TI_PRECEDES
     *  - SPIS_SPI_MODE_NATIONAL
    */
    uint32 submode;
     ...
     *  - SPIS_SPI_SCLK_CPHA0_CPOL0
     *  - SPIS_SPI_SCLK_CPHA0_CPOL1
     *  - SPIS_SPI_SCLK_CPHA1_CPOL0
     *  - SPIS_SPI_SCLK_CPHA1_CPOL1
    */
    uint32 sclkMode;
    ...
    */
     ...
     *  choices:
     *  - SPIS_BITS_ORDER_LSB_FIRST
     *  - SPIS_BITS_ORDER_MSB_FIRST
    */
    uint32 bitOrder;
     ...
     *  available choices: 
     *  - SPIS_SPI_TRANSFER_CONTINUOUS
     *  - SPIS_SPI_TRANSFER_SEPARATED
    */
     ...
     *  buffer.
     *  The SPIS_INTR _RX_NOT_EMPTY interrupt has to be enabled to 
    ...
    */
     ...
     *  SPIS_SPI_SLAVE_SELECT0 corresponds to slave select 0 
     *  polarity, bit SPIS_SPI_SLAVE_SELECT1 – slave select 1 
     *  polarity and so on. Polarity constants are: 
     *  - SPIS_SPI_SS_ACTIVE_LOW 
     *  - SPIS_SPI_SS_ACTIVE_HIGH
     *
     *  Ignored for all devices other than PSoC 4100 BLE / PSoC 4200 BLE / 
     *  PSoC 4100M / PSoC 4200M / PSoC 4200L / PSoC 4000S / PSoC 4100S / 
     *  PSoC Analog Coprocessor.
...
typedef struct
{
    ...
    */
     ...
     *  - SPIS_UART_PARITY_EVEN
     *  - SPIS_UART_PARITY_ODD
     *  - SPIS_UART_PARITY_NONE
    */
    uint32 parity;
    
    /** Determines the number of stop bits. The following defines are available 
     *  choices:
     *  - SPIS_UART_STOP_BITS_1
     *  - SPIS_UART_STOP_BITS_1_5
     *  - SPIS_UART_STOP_BITS_2
    */
    uint32 stopBits;
     ...
     *  is enabled:
     *  - SPIS_UART_IRDA_LP_OVS16
     *  - SPIS_UART_IRDA_LP_OVS32
     *  - SPIS_UART_IRDA_LP_OVS48
     *  - SPIS_UART_IRDA_LP_OVS96
     *  - SPIS_UART_IRDA_LP_OVS192
     *  - SPIS_UART_IRDA_LP_OVS768
     *  - SPIS_UART_IRDA_LP_OVS1536
    */
    uint32 oversample;
    ...
    */    
     ...
     *  - SPIS_UART_CTS_ACTIVE_LOW
     *  - SPIS_UART_CTS_ACTIVE_HIGH
     *
     *  Ignored for all devices other than PSoC 4100 BLE / PSoC 4200 BLE / 
     *  PSoC 4100M / PSoC 4200M / PSoC 4200L / PSoC 4000S / PSoC 4100S / 
     *  PSoC Analog Coprocessor.
    ...
    */
     ...
     *  - SPIS_UART_RTS_ ACTIVE_LOW 
     *  - SPIS_UART_RTS_ACTIVE_HIGH
     *
     *  Ignored for all devices other than PSoC 4100 BLE / PSoC 4200 BLE / 
     *  PSoC 4100M / PSoC 4200M / PSoC 4200L / PSoC 4000S / PSoC 4100S / 
     *  PSoC Analog Coprocessor.
...
/* SPI specific functions */
    ...
    /*******************************************************************************
    * Function Name: SPIS_SpiIsBusBusy
    ****************************************************************************//**
    *
...
/* UART specific functions */
    ...
    /* UART TX direction APIs */
        ...
        /*******************************************************************************
        * Function Name: SPIS_UartPutChar
        ****************************************************************************//**
        *
        ...
        *  Use SPIS_UART_MP_MARK to add a mark to create an address byte.
        *
...
***************************************/
    ...
    /* RX direction */
        ...
        #define SPIS_PutWordInRxBuffer(idx, rxDataByte) \
                do{                                                 \
                    SPIS_rxBufferInternal[(idx)] = ((uint8) (rxDataByte)); \
                }while(0)
    ...
    /* TX direction */
        ...
        #define SPIS_PutWordInTxBuffer(idx, txDataByte) \
                    do{                                             \
                        SPIS_txBufferInternal[(idx)] = ((uint8) (txDataByte)); \
                    }while(0)
...
#define SPIS_INTR_SPIM_TX_RESTORE   (SPIS_INTR_TX_OVERFLOW)

#define SPIS_INTR_SPIS_TX_RESTORE     (SPIS_INTR_TX_OVERFLOW | \
                                                 SPIS_INTR_TX_UNDERFLOW)
...
#define SPIS_INTR_UART_TX_RESTORE   (SPIS_INTR_TX_OVERFLOW  | \
                                                 SPIS_INTR_TX_UART_NACK | \
                                                 SPIS_INTR_TX_UART_DONE | \
                                                 SPIS_INTR_TX_UART_ARB_LOST)
...
#if (SPIS_UART_WAKE_ENABLE_CONST && SPIS_UART_RX_WAKEUP_IRQ)
    extern uint8 SPIS_skipStart;
#endif /* (SPIS_UART_WAKE_ENABLE_CONST && SPIS_UART_RX_WAKEUP_IRQ) */
