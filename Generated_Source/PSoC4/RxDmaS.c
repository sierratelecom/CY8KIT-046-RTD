...
uint8 RxDmaS_initVar = 0u;


/*******************************************************************************
* Function Name: RxDmaS_Start
********************************************************************************
*
* Summary:
...
{
    if(0u == RxDmaS_initVar)
    {
        RxDmaS_Init();
        RxDmaS_initVar = 1u;
    }
    CyDmaEnable();
    RxDmaS_SetSrcAddress(0, srcAddress);
    RxDmaS_SetDstAddress(0, dstAddress);
    RxDmaS_ValidateDescriptor(0);
    RxDmaS_ChEnable();
}


/*******************************************************************************
* Function Name: RxDmaS_Init
********************************************************************************
*
* Summary:
*  Initializes the DMA channel based on the parameters set in the component
*  customizer. It is not necessary to call RxDmaS_Init() because the
*  RxDmaS_Start() API calls this function, which is the preferred method
...
{
    CYDMA_CH_CTL_BASE.ctl[RxDmaS_CHANNEL] = RxDmaS_CHANNEL_CFG;
    CYDMA_DESCR_BASE.descriptor[RxDmaS_CHANNEL][0].ctl = RxDmaS_DESCR0_CFG;
    CYDMA_DESCR_BASE.descriptor[RxDmaS_CHANNEL][1].ctl = RxDmaS_DESCR1_CFG;

    /* If the Generate IRQ option is selected for descriptor 0 or 1, set the
    * channel bit field in the DMA interrupt mask register.
    */
    #if (0u != ((RxDmaS_DESCR0_CFG | RxDmaS_DESCR1_CFG) & CYDMA_GENERATE_IRQ))
        CYDMA_INTR_MASK_REG |= RxDmaS_CHANNEL_MASK;
    #endif /* (0u != ((RxDmaS_DESCR0_CFG | RxDmaS_DESCR1_CFG) & CYDMA_GENERATE_IRQ)) */
}
